	...
	public double withdraw(TextField input, String string) {
		...
		done1.setOnAction(e -> {
			get2 = textt.getText();
				...
				if(IsNum(textt)) {
					if ( Double.parseDouble(get2)>3000) {
						label3.setText("Cannot withdraw more than 3000 at once!!");
					} else if ( Double.parseDouble(get2)<3000) {
		...
		layout1.getChildren().addAll(done1,labell,textt,label2,label3);
